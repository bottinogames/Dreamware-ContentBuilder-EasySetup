...
This is a (nearly) drag and drop setup for a simple content builder setup

Included is a pre-made build script and depot description, a gitignore file so this can be dropped in your game repository without worry, as well as a few .bat files to get your started (if you're on windows).
...
There are a few lines of text you'll need to change in order for this to work for your project, so I'll list those out here:
 ...
 - `scripts\quickbuild.vdf`: Here `<app id>` and `<depot id>` will need to be set to their corresponding ID's as shown in Steamworks. `<branch name>` should be set to the name of the beta branch you'd like to have the build automatically pushed to.
    - Note: `<branch name>` cannot be set to "default", as Steam does not allow automatic pushes of builds to the defualt branch, so you must have a seperate branch to push to. Alternatively, if you do not want to build to go live to any branch, the `"Set Live"` line can be omitted.
